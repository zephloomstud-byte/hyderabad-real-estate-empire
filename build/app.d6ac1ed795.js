(() => {
  ...
  var EXTENDED_END_MONTH = HORIZONS[1].month;
  var horizonOf = (s) => {
    const y = s && s.horizonYear;
    const h = HORIZONS.find((x) => x.year === y);
    if (h) return h.month;
    return s && s.extended ? HORIZONS[1].month : END_MONTH;
  };
  var nextHorizon = (s) => {
  ...
  // src/data/history2020s.js
  var MACRO_2020S = {
    //          cpi%   gdp%   plr%   usdinr  credit  hydDemand
    2021: { cpi: 5.1, gdp: 9.1, plr: 9.3, usd: 74.2, credit: 0.72, demand: 1.35 },
    2022: { cpi: 6.7, gdp: 7.2, plr: 9.8, usd: 78.6, credit: 0.74, demand: 1.6 },
    2023: { cpi: 5.7, gdp: 8.2, plr: 10.5, usd: 82.6, credit: 0.7, demand: 1.55 },
    2024: { cpi: 4.9, gdp: 6.5, plr: 10.5, usd: 83.7, credit: 0.66, demand: 1.3 },
    2025: { cpi: 4.2, gdp: 6.5, plr: 9.8, usd: 86, credit: 0.7, demand: 1.35 },
    // Beyond here is extrapolation, not record.
    2026: { cpi: 4.5, gdp: 6.4, plr: 9.5, usd: 87.5, credit: 0.72, demand: 1.4 },
    2027: { cpi: 4.6, gdp: 6.3, plr: 9.6, usd: 89, credit: 0.7, demand: 1.42 },
    2028: { cpi: 4.8, gdp: 6, plr: 9.8, usd: 90.5, credit: 0.66, demand: 1.3 },
    2029: { cpi: 5, gdp: 5.6, plr: 10.2, usd: 92, credit: 0.58, demand: 1.1 },
    2030: { cpi: 4.7, gdp: 6.2, plr: 9.8, usd: 93.5, credit: 0.64, demand: 1.2 }
  };
  var SHOCKS_2020S = {
    303: { demand: 0.18, credit: 0.28 },
    // Apr 2020 — the national lockdown
    304: { demand: 0.22, credit: 0.3 },
    305: { demand: 0.35, credit: 0.36 },
    306: { demand: 0.5, credit: 0.44 },
    307: { demand: 0.62, credit: 0.52 },
    310: { demand: 0.85 },
    // Nov 2020 — festive recovery, rates at record lows
    315: { demand: 0.7, credit: 0.55 },
    // Apr 2021 — the second wave
    316: { demand: 0.65, credit: 0.55 },
    345: { demand: 1.2 },
    // Oct 2023 — pre-election pause
    347: { demand: 1.15, credit: 0.62 },
    // Dec 2023 — change of government in Telangana
    356: { demand: 1.05 }
    // Sep 2024 — HYDRAA demolitions unsettle the market
  };
  var TIMELINE_2020S = [
    {
      m: 303,
      tag: "ECONOMY",
      head: "National lockdown: every site in the country stops",
    ...
    },
    {
      m: 308,
      tag: "ECONOMY",
      head: "Repo rate cut to four per cent, the lowest ever recorded",
    ...
    },
    {
      m: 310,
      tag: "ECONOMY",
      head: "Housing comes back faster than anyone expected",
    ...
    },
    {
      m: 324,
      tag: "ECONOMY",
      head: "Prices are rising faster than at any time since 2007",
    ...
    },
    {
      m: 330,
      tag: "ECONOMY",
      head: "Rates turn: the Reserve Bank begins tightening",
    ...
    },
    {
      m: 336,
      tag: "INFRA",
      head: "Hyderabad office stock passes every city but Bangalore",
    ...
    },
    {
      m: 347,
      tag: "POLITICS",
      head: "Congress wins Telangana; Revanth Reddy becomes Chief Minister",
    ...
    },
    {
      m: 356,
      tag: "REGULATION",
      head: "HYDRAA begins demolishing construction on lake beds and buffer zones",
    ...
    },
    {
      m: 366,
      tag: "INFRA",
      head: "Future City and the Musi riverfront announced",
    ...
    },
    {
      m: 384,
      tag: "ECONOMY",
      head: "The market pauses for breath",
  ...
  var RENT_2020S = {
    ...
    office: [[2021, 3.45], [2022, 3.6], [2023, 3.75], [2025, 4], [2027, 4.3], [2030, 4.8]],
    retail: [[2021, 4.4], [2022, 4.8], [2023, 5.2], [2025, 5.8], [2027, 6.5], [2030, 7.5]],
    industrial: [[2021, 4.4], [2022, 4.8], [2023, 5.2], [2025, 6], [2027, 6.8], [2030, 8]]
  };
  var COST_2020S = [[2021, 5.3], [2022, 5.9], [2023, 6.2], [2025, 6.7], [2027, 7.3], [2030, 8.3]];
  var DUTY_2020S = [[2030, 0.06]];
  var FAR_2020S = [[2030, 5.5]];
  var CAP_2020S = [[2021, 0.075], [2023, 0.08], [2025, 0.078], [2030, 0.076]];
  var MATERIAL_2020S = {
    cement: [[2021, 3.4], [2022, 3.7], [2023, 3.8], [2025, 4], [2030, 4.8]],
  ...
  var WAGE_2020S = [[2021, 7.8], [2022, 8.5], [2023, 9.1], [2025, 10.2], [2027, 11.5], [2030, 13.5]];
  var SALARY_2020S = [[2021, 12.8], [2022, 14.2], [2023, 15.4], [2025, 17.5], [2027, 19.8], [2030, 23.5]];
  var UNAPPROVED_2020S = {
    // HYDRAA makes a defective approval genuinely dangerous rather than merely awkward.
    price: [[2021, 0.42], [2024, 0.38], [2025, 0.3], [2030, 0.28]],
    absorption: [[2021, 0.46], [2024, 0.42], [2025, 0.32], [2030, 0.3]]
  };
  var EXTENDED_END_MONTH2 = (2030 - 1995) * 12 + 11;

  // src/data/history2050.js
  var MACRO_2050 = {
    //          cpi%   gdp%   plr%   usdinr  credit  demand
    2031: { cpi: 4.8, gdp: 6, plr: 10, usd: 95.5, credit: 0.62, demand: 1.15 },
    2032: { cpi: 5.4, gdp: 4.8, plr: 10.8, usd: 98, credit: 0.48, demand: 0.88 },
    2033: { cpi: 5, gdp: 4.4, plr: 10.5, usd: 101, credit: 0.42, demand: 0.72 },
    2034: { cpi: 4.4, gdp: 5.6, plr: 9.6, usd: 103, credit: 0.58, demand: 0.92 },
    2035: { cpi: 4.2, gdp: 6.2, plr: 9.2, usd: 105, credit: 0.7, demand: 1.18 },
    2036: { cpi: 4.5, gdp: 6.4, plr: 9.3, usd: 107.5, credit: 0.76, demand: 1.34 },
    2037: { cpi: 5.2, gdp: 6.1, plr: 10, usd: 110, credit: 0.74, demand: 1.42 },
    2038: { cpi: 6.1, gdp: 4.6, plr: 11.2, usd: 114, credit: 0.44, demand: 1 },
    2039: { cpi: 5.5, gdp: 3.2, plr: 11, usd: 118.5, credit: 0.28, demand: 0.66 },
    2040: { cpi: 4.6, gdp: 4.4, plr: 9.8, usd: 121, credit: 0.38, demand: 0.62 },
    2041: { cpi: 4, gdp: 5.8, plr: 8.9, usd: 123, credit: 0.58, demand: 0.86 },
    2042: { cpi: 4.1, gdp: 6, plr: 8.8, usd: 125.5, credit: 0.7, demand: 1.1 },
    2043: { cpi: 4.3, gdp: 5.8, plr: 9, usd: 128, credit: 0.74, demand: 1.28 },
    2044: { cpi: 4.5, gdp: 5.5, plr: 9.2, usd: 131, credit: 0.74, demand: 1.38 },
    2045: { cpi: 4.7, gdp: 5.2, plr: 9.5, usd: 134, credit: 0.68, demand: 1.4 },
    ...
    2050: { cpi: 4.2, gdp: 4.4, plr: 9.2, usd: 149.5, credit: 0.66, demand: 1.2 }
  };
  var SHOCKS_2050 = {
    452: { credit: 0.34, demand: 0.72 },
    // Sep 2032 — the funding squeeze begins
    453: { credit: 0.3, demand: 0.66 },
    460: { demand: 0.58 },
    // Apr 2033 — the bottom
    521: { credit: 0.22, demand: 0.7 },
    // May 2038 — a large lender fails
    522: { credit: 0.18, demand: 0.58 },
    523: { credit: 0.18, demand: 0.52 },
    535: { demand: 0.48 },
    // Jun 2039 — nothing is registering
    536: { demand: 0.5 }
  };
  var TIMELINE_2050 = [
    {
      m: 444,
      tag: "INFRA",
      head: "Regional Ring Road opens on the northern arc",
    ...
    },
    {
      m: 460,
      tag: "ECONOMY",
      head: "The bottom, for those who can tell at the time",
    ...
    },
    {
      m: 478,
      tag: "INFRA",
      head: "Future City reaches critical mass",
    ...
    },
    {
      m: 492,
      tag: "ECONOMY",
      head: "Data centres become the largest single source of office-grade demand",
    ...
    },
    {
      m: 504,
      tag: "REGULATION",
      head: "Groundwater extraction restricted across the western corridor",
    ...
    },
    {
      m: 521,
      tag: "ECONOMY",
      head: "A large housing financier fails",
    ...
    },
    {
      m: 553,
      tag: "POLITICS",
      head: "Hyderabad crosses eighteen million",
    ...
    },
    {
      m: 576,
      tag: "INFRA",
      head: "Metro Phase III completes the orbital loop",
    ...
    },
    {
      m: 612,
      tag: "ECONOMY",
      head: "Construction labour is the binding constraint",
  ...
  var LOCALITIES_2030S = [
    {
      id: "futurecity",
      name: "Future City (Srisailam corridor)",
      zone: "south",
      base: 40,
      far: 1.75,
      airportCap: false,
      ...
      risk: { ASSIGNED_LAND: 0.28, LAYOUT_UNAPPROVED: 0.34, NO_ACCESS: 0.3, GOVT_CLAIM: 0.22 },
    ...
    {
      id: "rrrnorth",
      name: "Toopran / Gajwel (RRR north)",
      zone: "north",
      base: 30,
      far: 1.75,
      airportCap: false,
      from: 400,
      tags: ["agri", "far", "periphery"],
      liquidity: 0.15,
      prestige: 0.1,
    ...
    {
      id: "rrreast",
      name: "Bhongir / Choutuppal (RRR east)",
      zone: "east",
      base: 26,
      far: 1.75,
      airportCap: false,
      ...
      risk: { ASSIGNED_LAND: 0.3, LAYOUT_UNAPPROVED: 0.44, NO_ACCESS: 0.36, MISSING_HEIR: 0.2 },
  ...
  var RENT_2050 = {
    ...
    office: [[2033, 4.9], [2035, 5.2], [2037, 5.7], [2039, 5.6], [2041, 6], [2044, 6.8], [2047, 7.6], [2050, 8.6]],
  ...
  var MATERIAL_2050 = {
    cement: [[2033, 5], [2037, 5.8], [2041, 6.6], [2045, 7.6], [2050, 9]],
    steel: [[2033, 4.7], [2037, 5.4], [2041, 6], [2045, 7], [2050, 8.2]],
    sand: [[2033, 14.2], [2037, 16.8], [2041, 19.5], [2045, 23], [2050, 28]],
    brick: [[2033, 9.2], [2037, 10.8], [2041, 12.4], [2045, 14.5], [2050, 17.5]]
  };
  var FAR_HORIZON = (2050 - 1995) * 12 + 11;
  ...
  for (const loc of LOCALITIES) {
    const ext2 = TRACK_2020S[loc.id];
    if (ext2) loc.track = loc.track.concat(ext2.map(([year, v]) => ({ year, v })));
  ...
  for (const [use, ext2] of Object.entries(RENT_2020S)) {
    RENT_TRACK[use] = RENT_TRACK[use].concat(ext2.map(([year, v]) => ({ year, v })));
  }
  for (const [use, ext2] of Object.entries(RENT_2050)) {
    RENT_TRACK[use] = RENT_TRACK[use].concat(ext2.map(([year, v]) => ({ year, v })));
  }
  var DEFECTS = {
    ASSIGNED_LAND: {
  ...
  function initialIntel() {
    ...
    for (const l of LOCALITIES) {
      if (l.from) {
        intel[l.id] = { level: INTEL_NONE, fresh: 0 };
        continue;
      }
  ...
  function saveGame(s) {
    ...
    try {
      ...
      return {
        ok: false,
        bytes: json.length,
        error: quota ? `The save is ${Math.round(json.length / 1024)} KB and the browser has run out of room for it.` : "The browser refused to save: " + (e && e.message)
      };
    }
  }
  function loadGame() {
  ...
  var KEEP = { news: 400, ledger: 600, doneProjects: 40 };
  ...
  var ext = (arr) => arr.map(([year, v]) => ({ year, v }));
  ...
  var LAST_YEAR = Math.max(...Object.keys(MACRO).map(Number));
  var SHOCKS = {
    10: { credit: 0.22 },
    11: { credit: 0.24 },
    12: { credit: 0.28 },
  ...
  var TIMELINE_FULL = [...TIMELINE, ...TIMELINE_2020S, ...TIMELINE_2050];
  var REGIMES = {
...
A proper refurbishment is about ${money(cost)}.`,
  ...
  function makeLandOffer(m, s, rng, opts = {}) {
    const pool = opts.locality ? [BY_ID[opts.locality]] : localitiesAt(m).filter((l) => {
    ...
    let areaSqYd;
    const bigLeague = opts.township || s.stats.projectsDone >= 6 && s.netWorth > 5e8 && s.reputation > 40;
    ...
    else if (bigLeague) areaSqYd = Math.round(rng.range(2e3, 16e4) / 10) * 10;
  ...
  function makeDevAgreement(m, s, rng) {
    const standing = clamp(s.stats.projectsDone / 14, 0, 1) * 0.55 + clamp(s.reputation / 85, 0, 1) * 0.45;
    ...
    const areaSqYd = isAgri ? Math.round(SQYD_PER_ACRE * rng.range(1.5, 3 + reach * 95)) : Math.round(rng.range(400, 1800 + reach * 44e3) / 10) * 10;
    const ownerShare = clamp(
      rng.range(0.34, 0.5) - standing * 0.15 - (s.flags.cleanBooks ? 0.015 : 0),
      0.26,
      0.52
    );
    const defects = rollDefects(loc, rng, 0.1);
    const advance = Math.round(
      landRate(loc.id, m, s) * areaSqYd * rng.range(0.02, 0.08) / 1e3
    ) * 1e3;
    return {
      id: nextId(s, "O"),
      kind: "devagreement",
  ...
  // src/sim/listing.js
  var IPO_FROM_MONTH = 138;
  var REIT_FROM_MONTH = 289;
  function marketMultiple(s) {
    const demand = s.macro.demand;
    const credit = s.macro.credit;
    const base = clamp(0.55 + demand * 1.35 + (credit - 0.6) * 0.9, 0.5, 4.2);
    const quality = 1 + clamp(s.reputation, 0, 100) / 330 + (s.flags.cleanBooks ? 0.12 : -0.15) + (s.flags.internalAudit ? 0.06 : 0) + (s.staff.some((x) => x.role === "cfo") ? 0.07 : 0) + clamp(s.ratios.noi * 8 / Math.max(1, s.netWorth), 0, 0.35) - (s.flags.underInvestigation ? 0.35 : 0) - (s.flags.npa ? 0.2 : 0) - clamp((s.ratios.debtToAssets - 0.45) * 0.8, 0, 0.35);
    return clamp(base * quality, 0.35, 5.5);
  }
  function ipoReadiness(s) {
    const reasons = [];
  ...
  function ipoQuote(s, dilution = 0.25) {
  ...
  function marketCap(s) {
    if (!s.listed) return 0;
    return Math.max(0, s.netWorth) * marketMultiple(s);
  }
  function founderWealth(s) {
    if (!s.listed) return Math.max(0, s.netWorth) * (s.founderStake ?? 1);
    return marketCap(s) * (s.founderStake ?? 1);
  }
  function reitReadiness(s) {
    const reasons = [];
    const leased2 = s.assets.filter((a) => a.use !== "res" && a.occupancy > 0.7);
    const area = leased2.reduce((t, a) => t + a.sqFt, 0);
    const noi = leased2.reduce((t, a) => t + (a.lastNoi || 0) * 12, 0);
    ...
    if (noi < 15e7) reasons.push("Net operating income is too thin to support a public trust.");
    if (!s.flags.cleanBooks) reasons.push("A trust requires audited accounts and clean title on every asset.");
    return { ready: reasons.length === 0, reasons, leased: leased2, area, noi };
  }
  function reitQuote(s) {
  ...
  var idsChecked = /* @__PURE__ */ new WeakSet();
  function refresh(s) {
    if (!s.intel) s.intel = initialIntel();
    if (!s.seq || !idsChecked.has(s) && findDuplicateIds(s).length) {
      ...
      if (repaired.length) {
        s.news.push({
          ...
          body: `A fault in an earlier version reused ${repaired.length} internal reference${repaired.length === 1 ? "" : "s"} after a page reload, which made some buttons act on the wrong parcel or project \u2014 most visibly, land you clearly owned reporting that it had nothing left to build on. The numbering has been renumbered and cannot collide again. Nothing has been lost and no holding has changed hands.`
        });
      }
    }
    idsChecked.add(s);
  ...
  function equityRequirement(s) {
    ...
    const capability = clamp(
      track * 0.32 + standing * 0.18 + banked * 0.16 + organisation * 0.12 + recurring * 0.14 + (s.flags.cleanBooks ? 0.06 : 0) + (s.listed ? 0.1 : 0),
      0,
      1
    );
  ...
  function launchProject(s, parcelId, typeId, sqFt, mode, name) {
    ...
    if (s.cash < need) {
      return {
        ok: false,
        msg: `You cannot fund this. Total build cost ${money(est.budget)} over ${est.approvalMonths + est.months} months` + (committed > 0 ? `, on top of ${money(committed)} still to spend on projects already running` : "") + `. A firm of your standing should have about ${Math.round(share * 100)} per cent of that in hand \u2014 ${money(need)} \u2014 and you have ${money(s.cash)}. Build smaller, sell something, or raise money first.`
      };
    }
  ...
  function continuePast2020(s) {
    ...
    s.news.push(next.year === 2030 ? {
      m: s.month,
      tag: "NOTE",
      head: "The story continues",
      body: "April 2020. The lockdown is three weeks old, every site in the country is silent and nobody knows how long it lasts. From here to the end of 2025 the world follows what actually happened \u2014 the collapse, the cheapest home loans in Indian history, the boom that followed them, a change of government in Telangana and the demolitions that came with it. Beyond 2025 the numbers are a reasoned extrapolation and nothing more. You have until December 2030."
    } : {
      m: s.month,
      tag: "NOTE",
      head: "Twenty more years, and none of them real",
      body: "From here to December 2050 there is no record to be faithful to, so this is invention: a property market that behaves the way property markets behave. There are two serious downturns ahead \u2014 a cooling as the 2020s unwind, and a credit-driven correction of the kind that arrives about once a generation. Land does not go up every year and never has. A second ring road opens, the frontier moves out past it, and the western corridor stops being where the growth is. Treat none of it as a forecast."
    });
    refresh(s);
    return { ok: true, horizon: next };
  }
  function goPublic(s, dilution = 0.25) {
    ...
    s.news.push({
      ...
      body: `${money(q.raised)} raised by issuing ${Math.round(q.dilution * 100)} per cent of the company, ${money(q.costs)} of it eaten by bankers and lawyers. The market has put ${q.multiple.toFixed(2)} times net assets on the business \u2014 it pays for delivery, for recurring income and for accounts it believes, and it will stop paying for any of them the moment the cycle turns. Your ${Math.round(s.founderStake * 100)} per cent is worth ${money(q.founderValue)} on paper today.
...
You now answer to a board, to quarterly results and to anyone who buys a hundred shares. The firm cannot quietly stop and wait out a bad year any more.`
  ...
  function launchREIT(s) {
    ...
    let repaid = 0;
    for (const l of [...s.loans]) {
      if (l.assetId && ids.has(l.assetId)) {
        repaid += l.outstanding;
    ...
    s.news.push({
      ...
      body: `${(q.area / 1e6).toFixed(2)} million square feet producing ${money(q.noi)} a year, valued at a ${(q.yieldReq * 100).toFixed(1)} per cent yield \u2014 considerably finer than a private buyer would pay, because the units trade and the income is contracted. ${money(net)} released after repaying ${money(repaid)} of lease rental discounting and ${money(q.costs)} of issue costs. You keep a sponsor's ${Math.round(q.sponsorStake * 100)} per cent. Twenty-five years of building offices has just turned into capital you can deploy again.`
    });
...
Nothing has been notified in the gazette. Nothing has been surveyed. Nothing is certain, and everybody is behaving as though it is.`,
  ...
  // src/ui/app.js
  var BUILD_ID = "d6ac1ed795".startsWith("__") ? "source" : "d6ac1ed795";
  ...
  var toast = null;
  var advancing = false;
  var endingDismissed = false;
  var esc = (v) => String(v).replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c]);
  function boot() {
    const saved = loadGame();
    if (saved) {
      delete saved._idsChecked;
      compactState(saved);
      S = refresh(saved);
      persist();
      render();
  ...
  var lastSaveWarning = 0;
  function persist() {
    if (!S) return;
    const r = saveGame(S);
    if (!r.ok && Date.now() - lastSaveWarning > 15e3) {
  ...
  function render() {
    ...
    <div class="topbar">
      ...
      ${S.over ? `<button class="advance" id="reopen-ending">Simulation ended<small>Show the result</small></button>` : `<button class="advance" id="adv" ${S.pendingEvent || advancing ? "disabled" : ""}>
  ...
  `;
    ...
    if (advBtn) advBtn.onclick = () => {
      ...
      if (btn) {
        btn.disabled = true;
        btn.firstChild.textContent = "Working\u2026";
      }
      setTimeout(() => {
        try {
          advanceMonth(S);
  ...
  function capitalMarketsCard() {
    ...
    if (S.listed) {
      ...
      ${row("Market would pay", q.multiple.toFixed(2) + "\xD7 net assets")}
  ...
  var IPO_BLURB = "A private developer is worth what his assets are worth. A listed one is worth what the market says, and the market pays a multiple of book that runs from under one in a bust to four at a peak. This is how property fortunes are actually made, and unmade.";
  var REIT_BLURB = "A trust values let offices on contracted income at a yield institutions accept, materially finer than a private buyer pays. Selling the portfolio into one you sponsor releases most of that value as cash while you keep a quarter of it.";
  function listedSummaryCard(inner) {
    const mult = marketMultiple(S);
    const body = `${row("Market capitalisation", money(marketCap(S)))}
    ${row("Trading at", mult.toFixed(2) + "\xD7 net assets" + (mult < 1 ? " \u2014 below book" : ""))}
    ...
    return inner ? `<div class="card tight"><div class="spread"><b>Listed</b><span class="pill good">Public</span></div>${body}</div>` : `<div class="card"><h3>Listed company</h3>${body}</div>`;
  }
  ...
  function bindView() {
    ...
    if (ipoBtn) ipoBtn.onclick = () => {
      ...
      confirmModal({
        ...
        body: `The market is paying ${q.multiple.toFixed(2)} times net assets today, which values the business at ${money(q.postMoney)}. Issuing a quarter of it raises ${money(q.raised)}, of which ${money(q.costs)} goes to bankers and lawyers. Your remaining ${Math.round((1 - q.dilution) * 100)} per cent would be worth ${money(q.founderValue)} on paper.

What you give up is permanent. A quarter of every rupee the firm earns from here belongs to somebody else, you answer to a board and to quarterly results, and the multiple that makes this number large today is the one that takes it below book in the next crash. A public company cannot quietly stop and wait out a bad year the way a private one can.`,
    ...
    if (reitBtn) reitBtn.onclick = () => {
      ...
      confirmModal({
        ...
        body: `${(q.area / 1e6).toFixed(2)} million square feet producing ${money(q.noi)} a year, valued at ${money(q.grossValue)} on a ${pct(q.yieldReq)} yield. Selling three quarters of it into the trust releases about ${money(q.raised - q.costs)} after costs, and repays any lease rental discounting secured on those buildings.

You keep a sponsor's quarter and the income that goes with it. What you lose is the assets: the offices you spent years building stop being yours and become units other people trade.`,
...
The money goes now either way. It is not refunded if the application fails.`,
    ...
    if (dg) dg.onclick = () => {
      ...
      const zeroFree = S.parcels.filter((x) => x.owned && !x.consumed && freeSqYd(x) === 0).map((x) => `${x.label}: ${num(x.areaSqYd)} sq yd owned, ${num(x.usedSqYd || 0)} used`);
      const lines = [
      ...
      <div class="small muted" style="margin-top:8px">${dupes.length || mismatched.length ? "Problems found. Press <b>Repair internal references</b>, then try the Build button again." : "Nothing wrong with the references in this save. If a button is still misbehaving, send this text."}</div></div>`;
    };
    const rp = $("#repairnow");
    if (rp) rp.onclick = () => {
      reseedIds(S);
      ...
      refresh(S);
      persist();
      say(repaired.length ? `Repaired ${repaired.length} internal reference${repaired.length === 1 ? "" : "s"}. Try Build again.` : "Nothing needed repairing in this save.");
      render();
    };
    const ex = $("#export");
...
Not every introduction closes. Yours close more often the better you know the market and the people in it.`,
  ...
  function showEnding() {
    ...
    const onward = S.overReason === "time" || S.overReason === "extendedTime" ? nextHorizon(S) : null;
    const canContinue = !!onward;
    const verdict = () => {
    ...
    ${canContinue ? `<div class="body" style="padding-top:0">
      <div class="card tight" style="border-left:3px solid var(--accent)">
        ...
        ${onward.year === 2030 ? `<p class="small" style="margin:8px 0 0">The simulation ends in March 2020 because that is where the world it
        models ends. But the company is still standing, and you can carry it into the 2020s: the lockdown, the cheapest home loans in
        Indian history and the boom that followed them, a change of government in Telangana, and the demolitions that came after.
        Real ground to the end of 2025, a reasoned extrapolation after that, and five more years to run.</p>` : `<p class="small" style="margin:8px 0 0">Another twenty years, to December 2050, and none of it real. There is no record left to be
        faithful to, so what follows is a property market that behaves the way property markets behave: two serious downturns, a second
        ring road, and a frontier that moves out past it while the western corridor stops being where the growth is.
        Land does not go up every year. Treat none of it as a forecast.</p>`}
      </div>
    </div>` : ""}
    <div class="foot"><div class="inline">
      ${canContinue ? `<button class="btn" id="carryon">Carry on to ${esc(onward.label)}</button>` : ""}
